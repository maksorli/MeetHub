...
async def record_like(db: AsyncSession, background_tasks: BackgroundTasks, liker_id: int, liked_id: int):
...
async def notify_match(liker: Client, liked: Client, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(send_email, liker.email, "Взаимная симпатия", message_for_liker)
    background_tasks.add_task(send_email, liked.email, "Взаимная симпатия", message_for_liked)
